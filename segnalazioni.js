...
window.addEventListener('load', function() {
    ...
    if (!user.username) {
        alert('⚠️ Accesso Richiesto - Devi essere loggato per inviare una segnalazione!');
        window.location.href = 'index.html';
        return;
    }
    
    // ✅ Compila automaticamente i campi con i dati dell'utente loggato
    const reporterUsernameField = document.getElementById('reporterUsername');
    const reporterEmailField = document.getElementById('reporterEmail');
    
    if (reporterUsernameField) reporterUsernameField.value = user.username || '';
    if (reporterEmailField) reporterEmailField.value = user.email || '';
...
// ==================== SUBMIT DEL FORM - VERSIONE CORRETTA ====================
document.getElementById('reportForm').addEventListener('submit', async function(e) {
    ...
    showLoadingOverlay('📤 Caricamento prove su ImgBB...');
    ...
    try {
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
            ...
            if (file.isVideo) {
                showCustomNotification('warning', '⚠️ Video Ignorato', `${file.name} non può essere caricato (ImgBB supporta solo immagini)`);
                continue;
            }
            
            updateLoadingMessage(`📤 Caricamento ${i + 1}/${uploadedFiles.length}...`);
            
            const imgbbData = await uploadToImgBB(file);
            uploadedUrls.push(imgbbData);
    ...
    return false; // ✅ PREVIENE IL SUBMIT NATIVO
});
